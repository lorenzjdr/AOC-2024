from Day1.main import DayOne
from Day2.main import DayTwo
from Day3.main import DayThree
from Day4.main import DayFour

day_one = DayOne()
day_one.Solution()
print("Answer:", day_one.ans)

day_one.SolutionPartTwo()
print("Answer Part Two:", day_one.ans2)

day_two = DayTwo()
day_two.Solution()
print("Answer:", day_two.ans)

day_two.SolutionPartTwo()
print("Answer Part Two:", day_two.ans2)

day_three = DayThree()
day_three.Solution()
print("Answer:", day_three.ans)

day_three.SolutionPartTwo()
print("Answer Part Two:", day_three.ans2)